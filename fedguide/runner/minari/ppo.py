"""
Run Centralized PPO baseline for Minari environments.

This script trains a central PPO agent on Minari environments using on-policy rollouts.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Centralized PPO for Minari environments")
    ...
    print(f"\nCreating environment for Minari dataset: {args.dataset_id}...")
    try:
        import minari
        if args.env_name:
            print(f"Using provided environment name: {args.env_name}")
            env = gym.make(args.env_name)
        else:
            try:
                ds = minari.load_dataset(args.dataset_id, download=args.download)
                ...
                dataset_id_lower = args.dataset_id.lower()
                if "pointmaze" in dataset_id_lower:
                    ...
                    raise ValueError(f"Could not infer environment from dataset_id: {args.dataset_id}. "
