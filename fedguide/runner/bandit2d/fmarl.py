"""
Run FMARL federated training for 2D Bandit environment.

This module trains a federated FMARL agent on Bandit2D using Flower simulation.
"""
...
from fedguide.baselines.fmarl.server import run_fmarl_server
...
def main():
    """Main entry point for FMARL federated training on Bandit2D."""
    parser = argparse.ArgumentParser(description="FMARL federated training for Bandit2D")
    ...
    parser.add_argument("--lambda_local", type=float, default=0.05,
                       help="Local KL penalty coefficient (for old policy snapshot)")
    ...
    parser.add_argument("--metrics_dir", type=str, default="./metrics/bandit2d_fmarl",
    ...
    client_fn = client_fn_builder(
        ...
        run_name=args.run_name or "bandit2d-fmarl",
        metrics_collector=metrics_collector,
        num_clients=args.num_clients,
    )
    
    print(f"Starting FMARL training:")
    ...
    # Save results
    save_training_results(history, metrics_collector, args.metrics_dir, "fmarl")
