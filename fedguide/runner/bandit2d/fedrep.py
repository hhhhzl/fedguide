"""
Run FedRep federated training for 2D Bandit environment.

This module trains a federated FedRep agent on Bandit2D using Flower simulation.
FedRep uses encoder/head separation where only encoder parameters are aggregated.
"""
...
from fedguide.baselines.fedrep.server import run_fedrep_server
...
def main():
    """Main entry point for FedRep federated training on Bandit2D."""
    parser = argparse.ArgumentParser(description="FedRep federated training for Bandit2D")
    ...
    # Training args (standard PPO, no KL penalties)
    ...
    parser.add_argument("--metrics_dir", type=str, default="./metrics/bandit2d_fedrep",
    ...
    client_fn = client_fn_builder(
        ...
        run_name=args.run_name or "bandit2d-fedrep",
        metrics_collector=metrics_collector,
        num_clients=args.num_clients,
    )
    
    print(f"Starting FedRep training:")
    print(f"  Environment: Bandit2D")
    print(f"  Clients: {args.num_clients}")
    print(f"  Rounds: {args.rounds}")
    print(f"  Steps per round: {args.n_steps}")
    print(f"  Architecture: Encoder (shared) + Head (local) + Value (local)")
    
    # Run FedRep server
    history = run_fedrep_server(
        client_fn=client_fn,
    ...
    # Save results
    save_training_results(history, metrics_collector, args.metrics_dir, "fedrep")
