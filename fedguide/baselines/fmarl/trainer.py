"""
FMARL Trainer Implementation

This module implements the PPO trainer for FMARL with old policy synchronization
and client weight tracking for sequential federated training.
"""
...
class FMARLTrainer:
    """
    FMARL Trainer implementing PPO with old policy synchronization.
    
    Key features:
    - Syncs old policy before training (via agent.sync_old_policy())
    - Resets client weight and optimizer at start of each round
    - Tracks number of timesteps (n_steps) for client weight in aggregation
    """
    ...
    def __init__(
        ...
        # Track number of timesteps collected in current round (for client weight)
        self.current_round_timesteps = 0
    ...
    def train_one_round(self) -> Dict[str, float]:
        ...
        FMARL-specific steps:
        1. Sync old policy (via agent.sync_old_policy())
        2. Reset client weight (via agent.reset_client_weight())
        3. Reset optimizer (via agent.reset_optimizer())
        4. Collect rollouts and train
        5. Update client weight with number of timesteps
        """
        t0 = time.time()
        
        # FMARL: Sync old policy before training (for KL divergence computation)
        if hasattr(self.agent, 'sync_old_policy'):
            self.agent.sync_old_policy()
        
        # FMARL: Reset client weight at start of round
        if hasattr(self.agent, 'reset_client_weight'):
            self.agent.reset_client_weight()
        
        # FMARL: Reset optimizer at start of round (optional, but matches original FMARL)
        if hasattr(self.agent, 'reset_optimizer'):
            self.agent.reset_optimizer()
        
        # Collect rollouts
        states, actions, logps_old, returns, extras = self._rollout()
        
        # Track number of timesteps collected (for client weight)
        self.current_round_timesteps = states.size(0)
        
        # Update agent's client weight
        if hasattr(self.agent, 'update_client_weight'):
            self.agent.update_client_weight(self.current_round_timesteps)
    ...
    def save_eval(self, cid: str, rnd: int, outdir = "./results/fmarl") -> bool:
