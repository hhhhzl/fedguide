"""
FMARL Agent Implementation

This module implements the FMARL agent with old policy synchronization
for sequential federated training.
"""
...
class FMARLAgent(nn.Module):
    """
    FMARL Agent implementing sequential federated training with old policy synchronization.
    
    Key features:
    - Maintains old policy snapshot for KL divergence computation
    - Tracks client weight (number of timesteps) for weighted aggregation
    - Supports optimizer reset for sequential training
    """
    ...
    def __init__(
        ...
        self.old_policy = PolicyNetwork(state_dim, action_dim, hidden_dim).to(self.device)
        self.old_log_std = nn.Parameter(torch.zeros(action_dim, device=self.device))
        
        # Copy initial parameters to old policy
        self.old_policy.load_state_dict(self.policy.state_dict())
        self.old_log_std.data = self.log_std.data.clone()
        ...
        # Client weight tracking (number of timesteps)
        self.num_timesteps_seen = 0
        
        # Clamp log_std to reasonable range
        self.log_std.data.clamp_(-5.0, 2.0)
    
    # ========= FMARL-specific methods =========
    def sync_old_policy(self):
        """
        Save current policy as old policy snapshot.
        Called before training each round to compute KL divergence.
        """
        self.old_policy.load_state_dict(self.policy.state_dict())
        self.old_log_std.data = self.log_std.data.clone()
    ...
    def reset_optimizer(self):
        """
        Reset optimizer state (clears momentum, etc.).
        Called at the start of each federated round.
        """
        self.optimizer = torch.optim.Adam(
            list(self.policy.parameters()) + [self.log_std] + list(self.value_fn.parameters()),
            lr=self.lr
    ...
    def get_client_weight(self) -> int:
        """
        Get client weight (number of timesteps seen).
        Used for weighted aggregation in federated learning.
        """
        return self.num_timesteps_seen
    
    def reset_client_weight(self):
        """
        Reset client weight counter.
        Called at the start of each federated round.
        """
        self.num_timesteps_seen = 0
    
    def update_client_weight(self, num_timesteps: int):
        """
        Update client weight with number of timesteps from current round.
        """
        self.num_timesteps_seen = num_timesteps
    ...
    def compute_kl_divergence_old(self, states: torch.Tensor) -> torch.Tensor:
        """
        Compute KL divergence between current policy and old policy snapshot.
        KL(π_current || π_old) for Gaussian policies.
        """
        # Current policy distribution
        mean_current = self.policy(states)
        std_current = torch.exp(self.log_std).clone()
        
        # Old policy distribution (detached)
        with torch.no_grad():
            mean_old = self.old_policy(states)
            std_old = torch.exp(self.old_log_std).clone()
        
        # KL divergence for diagonal Gaussian
        var_current = std_current.pow(2) + 1e-8
        var_old = std_old.pow(2) + 1e-8
        
        kl = (
            torch.log(std_old / std_current)
            + (var_current + (mean_current - mean_old).pow(2)) / (2 * var_old)
            - 0.5
        )
    ...
    def update(
        ...
        Args:
            ...
            lambda_local: Weight for KL(π || π_old_policy_snapshot)
        """
        ...
        for _ in range(epochs):
            ...
            for st in range(0, N, minibatch_size):
                ...
                # Local KL: divergence from old policy snapshot (if lambda_local > 0)
                kl_local = torch.tensor(0.0, device=self.device)
                if lambda_local > 0.0:
                    kl_local = self.compute_kl_divergence_old(mb_s)
    ...
    def to(self, device: str):
        ...
        self.old_log_std.data = self.old_log_std.data.to(self.device)
