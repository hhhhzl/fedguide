"""Generate reacher hetero metadata.json files.

  A — main/canonical reacher heterogeneity (numerical only: per-client goal
      region + action noise + reward scale). Written to data/reacher/metadata.json.
...
B and C are used for ablation experiments under configs/reacher/ablation/.
...
def main():
    n_clients = 8
    seed = 42
    # A is the main reacher hetero metadata → metadata.json.
    # B and C are ablations → metadata_B.json / metadata_C.json.
    for level in ("A", "B", "C"):
        ...
        out_name = "metadata.json" if level == "A" else f"metadata_{level}.json"
