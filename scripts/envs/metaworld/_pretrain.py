"""Pretrain DiffusionGuidance + SDICE_Critic for federated MetaWorld ML10.

Each client = one ML10 task (10 clients total). For each client we:
  1. Instantiate the task's env via metaworld.ML10().train_classes
  2. Collect ``rollout_steps`` (s, a, r, s', d) tuples using either:
       - ``--behaviour scripted`` (default): MetaWorld's scripted expert policy
       - ``--behaviour random``: uniform action_space.sample()
  3. Pretrain DiffusionGuidance on the (s, a) pairs and SDICE_Critic on the
     transitions. Outputs land at:
       ./model/models_prior/MetaWorld/client_{i}/final/{torch_prior.pth,
                                                       guidance_sdice.pth}

Usage:
    python scripts/envs/metaworld/_pretrain.py \\
        --num_clients 10 --behaviour scripted \\
        --rollout_steps 5000 --n_behavior_epochs 40 \\
...
def _make_metaworld_env(task_name: str, seed: int):
    import metaworld
    ml10 = metaworld.ML10(seed=seed)
    if task_name in ml10.train_classes:
        env_cls = ml10.train_classes[task_name]
        tasks_for_name = [t for t in ml10.train_tasks if t.env_name == task_name]
    else:
        env_cls = ml10.test_classes[task_name]
        tasks_for_name = [t for t in ml10.test_tasks if t.env_name == task_name]
    env = env_cls()
    if not tasks_for_name:
        raise RuntimeError(f"no MetaWorld task instances for {task_name}")
    env.set_task(tasks_for_name[0])
    return env


def _scripted_policy_for(task_name: str):
    from metaworld.policies import ENV_POLICY_MAP
    cls = ENV_POLICY_MAP.get(task_name)
    if cls is None:
        return None
    return cls()


def _collect_rollouts(env, total_steps: int, behaviour: str, task_name: str, seed: int):
    s_buf, a_buf, r_buf, s2_buf, d_buf = [], [], [], [], []
    obs, _ = env.reset(seed=seed)
    rng = np.random.RandomState(seed)
    policy = _scripted_policy_for(task_name) if behaviour == "scripted" else None
    if behaviour == "scripted" and policy is None:
        warnings.warn(f"no scripted policy for {task_name}; falling back to random")
    for _ in range(total_steps):
        if policy is not None:
            a = policy.get_action(obs)
            # Add a little noise to broaden coverage.
            a = np.clip(a + rng.normal(0, 0.05, size=a.shape), -1.0, 1.0)
        else:
            a = env.action_space.sample()
        out = env.step(a)
        obs2, r, term, trunc, _ = out
...
def main():
    ...
    print(f"[pretrain_metaworld] using metadata: {metadata_path}")
    print(f"[pretrain_metaworld] device: {device}")
    ...
    for offset in range(args.num_clients):
        ...
        cfg = clients[cid]
        task_name = cfg["task"]
        print(f"\n[Pretrain] MetaWorld client {cid} | task={task_name} | behaviour={args.behaviour}")
        ...
        s, a, r, s2, d = _collect_rollouts(env, args.rollout_steps, args.behaviour, task_name, seed=args.seed + cid)
    ...
    print("\n[Pretrain] All MetaWorld clients done.")
