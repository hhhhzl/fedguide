"""Run all bandit2d baseline configs (federated baselines only).
...
``metrics/bandit2d_phase1/<algo>/seed_<i>/``.

Configs are loaded from ``configs/bandit2d/baseline/``:
    fedavg, fedkl, fedrep, fedmomentum, fmarl, fedrl_ddpg

Usage:
    python scripts/envs/bandit2d/run_baselines.py --seeds 0 --rounds 60
    python scripts/envs/bandit2d/run_baselines.py --only fedavg,fedkl
"""
...
JOBS = [
    ...
    ("fedavg",      "fedkl",        "configs/bandit2d/baseline/fedavg.yaml"),       # FedAvg = FedKL with λ=0
    ("fedkl",       "fedkl",        "configs/bandit2d/baseline/fedkl.yaml"),
    ("fedrep",      "fedrep",       "configs/bandit2d/baseline/fedrep.yaml"),
    ("fedmomentum", "fedmomentum",  "configs/bandit2d/baseline/fedmomentum.yaml"),
    ("fmarl",       "fmarl",        "configs/bandit2d/baseline/fmarl.yaml"),
    ("fedrl",       "fedrl",        "configs/bandit2d/baseline/fedrl_ddpg.yaml"),
]
...
def main():
    ...
    for key, s, rc, dur in summary:
        ...
        print(f"{key:>14s}  seed={s}  status={status}  time={dur:6.1f}s")
