"""Run the 3 main FedGuide variants on reacher.

Variants in ``configs/reacher/main/``:
...
``./model/models_prior/Reacher/client_{0..7}/final/{torch_prior.pth,guidance_sdice.pth}``
are missing, it runs the diffusion pretrain before launching any sweep
job. Pass ``--skip_pretrain`` to bypass.

Usage:
    python scripts/envs/reacher/run_main.py --seeds 0 --rounds 50
    python scripts/envs/reacher/run_main.py --only fedguide_prior,fedguide_pg
"""
...
JOBS = [
    ...
    ("fedguide_prior", "fedguide", "configs/reacher/main/fedguide_prior.yaml"),
    ("fedguide_pg",    "fedguide", "configs/reacher/main/fedguide_pg.yaml"),
    ("fedguide_all",   "fedguide", "configs/reacher/main/fedguide_all.yaml"),
]


def _reacher_priors_present(num_clients: int = 8,
                            root: str = "./model/models_prior") -> bool:
    base = _PROJECT_ROOT / root / "Reacher"
...
def ensure_pretrain(num_clients: int = 8,
    ...
    """Run the reacher diffusion pretrain if the ckpts aren't on disk."""
    if _reacher_priors_present(num_clients=num_clients, root=save_root):
        print(f"[pretrain] already present at {save_root}/Reacher/")
        return
    print(f"[pretrain] running diffusion pretrain → {save_root}/Reacher/ ...")
    cmd = [
        sys.executable,
        str(_PROJECT_ROOT / "scripts" / "envs" / "reacher" / "_pretrain.py"),
...
def ensure_bc_pretrain(num_clients: int = 8,
    ...
    if _bc_policies_present(num_clients=num_clients, root=save_root):
        print(f"[bc_pretrain] already present at {save_root}/Reacher/")
        return
    print(f"[bc_pretrain] running BC pretrain → {save_root}/Reacher/ ...")
    cmd = [
        sys.executable,
        str(_PROJECT_ROOT / "scripts" / "envs" / "reacher" / "_bc_pretrain.py"),
        "--num_clients", str(num_clients),
        "--save_root", save_root,
        "--device", device,
        "--behaviour", behaviour,
...
def run_one(key: str, algorithm: str, config_path: str, seed: int, rounds: int,
    ...
    out_models = _PROJECT_ROOT / "model" / "policy" / "reacher_phase1" / key / f"seed_{seed}"
...
def main():
    ...
    ap.add_argument("--metrics_root", type=str, default="metrics/reacher_phase1")
    ap.add_argument("--log_dir", type=str, default="logs/reacher_phase1")
