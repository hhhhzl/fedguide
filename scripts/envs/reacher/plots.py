"""Reacher Phase-1 plotting utilities (mirrors bandit2d/plots.py).
...
Usage:
    python scripts/envs/reacher/plots.py curves
    python scripts/envs/reacher/plots.py summary
    python scripts/envs/reacher/plots.py list_videos
    python scripts/envs/reacher/plots.py all          # curves + summary + list_videos
"""
...
def _plot_curves(args):
    ...
    fig.suptitle(f"Reacher Phase-1 (8 hetero clients) — seed={args.seed}",
                 fontsize=12)
...
def _summary(args):
    root = _PROJECT_ROOT / args.root
    out_lines = ["# Reacher Phase-1 summary", ""]
    out_lines.append(
...
def _list_videos(args):
    ...
    if not root.exists():
        ...
        return
    lines = ["# Reacher Phase-1 — final-round per-client videos", ""]
...
def main():
    ...
    ap.add_argument("--root", type=str, default="metrics/reacher_phase1")
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--seeds", type=int, nargs="+", default=[0])
    ap.add_argument("--rounds", type=int, default=50)
    ap.add_argument("--algos", type=str, nargs="+", default=_DEFAULT_ALGOS)
    ap.add_argument("--video_root", type=str, default="plots/reacher_phase1")

    sub = ap.add_subparsers(dest="cmd", required=True)
    cu = sub.add_parser("curves")
    cu.add_argument("--out", type=str, default="plots/reacher_phase1/reward_curves.png")
    cu.set_defaults(fn=_plot_curves)

    su = sub.add_parser("summary")
    su.add_argument("--out", type=str, default="metrics/reacher_phase1/SUMMARY.md")
    su.set_defaults(fn=_summary)

    lv = sub.add_parser("list_videos")
    lv.add_argument("--out", type=str, default="plots/reacher_phase1/VIDEOS.md")
    ...
    if args.cmd == "all":
        args.out = "plots/reacher_phase1/reward_curves.png"
        _plot_curves(args)
        args.out = "metrics/reacher_phase1/SUMMARY.md"
        _summary(args)
        args.out = "plots/reacher_phase1/VIDEOS.md"
        _list_videos(args)
    else:
        args.fn(args)
