"""Pretrain DiffusionGuidance + SDICE_Critic for federated Reacher.
...
``CustomizedReacherEnv`` (uses the heterogeneity from
``data/reacher/metadata.json``: per-client goal region, action noise,
...
Outputs:
    ./model/models_prior/Reacher/client_{i}/final/torch_prior.pth
    ./model/models_prior/Reacher/client_{i}/final/guidance_sdice.pth

Usage:
    # Quick smoke test (1 client, random behaviour, 200 epochs, small buffer):
    python scripts/envs/reacher/pretrain_reacher.py \
...
def main():
    ...
    ap.add_argument("--metadata_path", type=str, default="data/reacher/metadata.json")
    ...
    ap.add_argument("--env_name", type=str, default="Reacher")
    ...
    print(f"[pretrain_reacher] using metadata: {metadata_path}")
    print(f"[pretrain_reacher] device: {device}")

    for offset in range(args.num_clients):
        client_id = args.first_client_id + offset
        print(f"\n[Pretrain] Reacher client {client_id} | behaviour={args.behaviour}")

        env = make_hetero_reacher_env_from_metadata(
