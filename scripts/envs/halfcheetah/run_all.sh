#!/usr/bin/env bash
# Run the full halfcheetah Phase-1 pipeline:
...
#
# Usage:
#   bash scripts/envs/halfcheetah/run_all.sh                        # 1 seed × 50 rounds
#   bash scripts/envs/halfcheetah/run_all.sh --seeds "0 1" --rounds 50
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
if [[ "$SKIP_BASE" -eq 0 ]]; then
  ...
  python scripts/envs/halfcheetah/run_baselines.py \
    --seeds "${SEED_ARR[@]}" --rounds "$ROUNDS" $ONLY_BASE
fi

if [[ "$SKIP_MAIN" -eq 0 ]]; then
  echo "===== [run_all] main FedGuide ====="
  python scripts/envs/halfcheetah/run_main.py \
