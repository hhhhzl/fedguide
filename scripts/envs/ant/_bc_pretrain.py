"""BC pretrain (warm-start) for Ant FedGuide policies.

Ant's prior is a state-conditional UNet1D diffusion model — there's no
single μ to copy into the policy bias the way bandit2d's GaussianBehaviorPrior
allows. The "D-fix" warm-start equivalent for ant is **behavior cloning**:
...
Usage:
    python scripts/envs/ant/_bc_pretrain.py --num_clients 8 \\
...
from scripts.envs.ant._pretrain import (
