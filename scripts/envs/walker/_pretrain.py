"""Pretrain DiffusionGuidance + SDICE_Critic for federated Walker2D.
...
the federated Walker2D env (uses the heterogeneity from
``data/walker/metadata.json``: per-client goal region, action noise,
...
Outputs:
    ./model/models_prior/Walker2D/client_{i}/final/torch_prior.pth
    ./model/models_prior/Walker2D/client_{i}/final/guidance_sdice.pth

Usage:
    # Quick smoke test (1 client, random behaviour, 200 epochs, small buffer):
    python scripts/envs/walker/_pretrain.py \
...
def main():
    ...
    ap.add_argument("--d4rl_variant", type=str, default="walker2d-medium-v2",
                    help="D4RL dataset to load when --behaviour=d4rl. "
                         "Common choices: walker2d-medium-v2, walker2d-expert-v2, walker2d-medium-replay-v2")
    ...
    print(f"[pretrain_walker] using metadata: {metadata_path}")
    print(f"[pretrain_walker] device: {device}")

    for offset in range(args.num_clients):
        client_id = args.first_client_id + offset
        print(f"\n[Pretrain] Walker2D client {client_id} | behaviour={args.behaviour}")
