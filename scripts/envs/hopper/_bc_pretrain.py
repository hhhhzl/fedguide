"""BC pretrain (warm-start) for Hopper FedGuide policies.

Hopper's prior is a state-conditional UNet1D diffusion model — there's no
single μ to copy into the policy bias the way bandit2d's GaussianBehaviorPrior
allows. The "D-fix" warm-start equivalent for hopper is **behavior cloning**:
...
Usage:
    python scripts/envs/hopper/_bc_pretrain.py --num_clients 8 \\
...
from scripts.envs.hopper._pretrain import (
