"""Pretrain DiffusionGuidance + SDICE_Critic for federated Hopper.
...
the federated Hopper env (uses the heterogeneity from
``data/hopper/metadata.json``: per-client goal region, action noise,
...
Outputs:
    ./model/models_prior/Hopper/client_{i}/final/torch_prior.pth
    ./model/models_prior/Hopper/client_{i}/final/guidance_sdice.pth

Usage:
    # Quick smoke test (1 client, random behaviour, 200 epochs, small buffer):
    python scripts/envs/hopper/_pretrain.py \
...
def main():
    ...
    ap.add_argument("--metadata_path", type=str, default="data/hopper/metadata.json")
    ...
    ap.add_argument("--d4rl_variant", type=str, default="hopper-medium-v2",
                    help="D4RL dataset to load when --behaviour=d4rl. "
                         "Common choices: hopper-medium-v2, hopper-expert-v2, hopper-medium-replay-v2")
    ...
    ap.add_argument("--env_name", type=str, default="Hopper")
    ...
    print(f"[pretrain_hopper] using metadata: {metadata_path}")
    print(f"[pretrain_hopper] device: {device}")

    for offset in range(args.num_clients):
        client_id = args.first_client_id + offset
        print(f"\n[Pretrain] Hopper client {client_id} | behaviour={args.behaviour}")
